...
class MethodBindingsServiceTest extends AbstractClientServerTest {
    ...
    try (MethodBindings bindings = new MethodBindings()) {
      ...
      assertEquals(
          ...
          assertThrows(
              ...
              .getStatusCode());
      CallMethodResult result = call(condition.getNode(), method);
      assertTrue(
          result.getStatusCode().isGood()
              || result
                  .getStatusCode()
                  .equals(new StatusCode(StatusCodes.Bad_ConditionAlreadyEnabled)));
    } finally {
  ...
  private final class BindingNamespace extends ManagedNamespaceWithLifecycle {
    ...
    Condition createCondition() throws UaException {
      var request =
          org.eclipse.milo.opcua.sdk.server.nodes.instantiation.InstantiationRequest.of(
                  org.eclipse.milo.opcua.sdk.server.model.objects.AcknowledgeableConditionTypeNode
                      .class,
              ...
              .methodInstantiation(
                  org.eclipse.milo.opcua.sdk.server.nodes.instantiation.MethodInstantiation.SHARE)
              .build();
      var node = getServer().getNodeInstantiator().instantiate(request).root();
      node.setEventType(NodeIds.AcknowledgeableConditionType);
      node.setSeverity(
          org.eclipse.milo.opcua.stack.core.types.builtin.unsigned.Unsigned.ushort(500));
      return org.eclipse.milo.opcua.sdk.server.conditions.AcknowledgeableCondition.adopt(
