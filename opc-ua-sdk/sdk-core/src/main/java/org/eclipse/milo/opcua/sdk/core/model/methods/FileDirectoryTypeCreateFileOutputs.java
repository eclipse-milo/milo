...
/**
 * Outputs of the <code>CreateFile</code> Method, in declaration order.
 *
 ...
 */
@NullMarked
public interface FileDirectoryTypeCreateFileOutputs {
  /**
  ...
  static FileDirectoryTypeCreateFileOutputs of(
      @Nullable NodeId fileNodeId, @Nullable UInteger fileHandle) {
    @Nullable NodeId fileNodeIdValue = FileDirectoryTypeCreateFileOutputs.snapshot(fileNodeId);
    @Nullable UInteger fileHandleValue = FileDirectoryTypeCreateFileOutputs.snapshot(fileHandle);
    return new FileDirectoryTypeCreateFileOutputs() {
      @Override
      public @Nullable NodeId fileNodeId() {
        return FileDirectoryTypeCreateFileOutputs.snapshot(fileNodeIdValue);
      }

      @Override
      public @Nullable UInteger fileHandle() {
        return FileDirectoryTypeCreateFileOutputs.snapshot(fileHandleValue);
      }
    };
  }
