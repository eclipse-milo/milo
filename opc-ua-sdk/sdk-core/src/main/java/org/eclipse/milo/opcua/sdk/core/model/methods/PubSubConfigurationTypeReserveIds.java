package org.eclipse.milo.opcua.sdk.core.model.methods;

import java.util.Arrays;
import java.util.Objects;
import org.eclipse.milo.opcua.stack.core.NamespaceTable;
import org.eclipse.milo.opcua.stack.core.UaException;
import org.eclipse.milo.opcua.stack.core.encoding.EncodingContext;
import org.eclipse.milo.opcua.stack.core.types.builtin.ExpandedNodeId;
import org.eclipse.milo.opcua.stack.core.types.builtin.LocalizedText;
import org.eclipse.milo.opcua.stack.core.types.builtin.QualifiedName;
import org.eclipse.milo.opcua.stack.core.types.builtin.Variant;
import org.eclipse.milo.opcua.stack.core.types.builtin.unsigned.UInteger;
import org.eclipse.milo.opcua.stack.core.types.builtin.unsigned.UShort;
import org.eclipse.milo.opcua.stack.core.types.structured.Argument;
import org.eclipse.milo.opcua.stack.core.util.Namespaces;
import org.jspecify.annotations.Nullable;

/**
 * Declared arguments and typed wire conversion for the ReserveIds Method of
 * PubSubConfigurationType.
 *
 * @see <a href="https://reference.opcfoundation.org/v105/Core/docs/Part14/9.1.3/#9.1.3.7.5">Model
 *     documentation</a>
 */
public final class PubSubConfigurationTypeReserveIds {
  private PubSubConfigurationTypeReserveIds() {}

  /** Returns the BrowseName resolved in the caller's namespace table. */
  public static QualifiedName browseName(NamespaceTable namespaceTable) {
    return new QualifiedName(
        Objects.requireNonNull(
            namespaceTable.getIndex("http://opcfoundation.org/UA/"),
            "missing Method BrowseName namespace"),
        "ReserveIds");
  }

  /** Returns the declared input Arguments resolved in the caller's namespace table. */
  public static Argument[] inputArguments(NamespaceTable namespaceTable) {
    return new Argument[] {
      new Argument(
          "TransportProfileUri",
          ExpandedNodeId.of(Namespaces.OPC_UA, 12L)
              .toNodeId(namespaceTable)
              .orElseThrow(() -> new IllegalArgumentException("missing namespace for i=12")),
          -1,
          new UInteger[] {},
          new LocalizedText(null, "")),
      new Argument(
          "NumReqWriterGroupIds",
          ExpandedNodeId.of(Namespaces.OPC_UA, 5L)
              .toNodeId(namespaceTable)
              .orElseThrow(() -> new IllegalArgumentException("missing namespace for i=5")),
          -1,
          new UInteger[] {},
          new LocalizedText(null, "")),
      new Argument(
          "NumReqDataSetWriterIds",
          ExpandedNodeId.of(Namespaces.OPC_UA, 5L)
              .toNodeId(namespaceTable)
              .orElseThrow(() -> new IllegalArgumentException("missing namespace for i=5")),
          -1,
          new UInteger[] {},
          new LocalizedText(null, ""))
    };
  }

  /** Returns the declared output Arguments resolved in the caller's namespace table. */
  public static Argument[] outputArguments(NamespaceTable namespaceTable) {
    return new Argument[] {
      new Argument(
          "DefaultPublisherId",
          ExpandedNodeId.of(Namespaces.OPC_UA, 24L)
              .toNodeId(namespaceTable)
              .orElseThrow(() -> new IllegalArgumentException("missing namespace for i=24")),
          -1,
          new UInteger[] {},
          new LocalizedText(null, "")),
      new Argument(
          "WriterGroupIds",
          ExpandedNodeId.of(Namespaces.OPC_UA, 5L)
              .toNodeId(namespaceTable)
              .orElseThrow(() -> new IllegalArgumentException("missing namespace for i=5")),
          1,
          new UInteger[] {UInteger.valueOf(0L)},
          new LocalizedText(null, "")),
      new Argument(
          "DataSetWriterIds",
          ExpandedNodeId.of(Namespaces.OPC_UA, 5L)
              .toNodeId(namespaceTable)
              .orElseThrow(() -> new IllegalArgumentException("missing namespace for i=5")),
          1,
          new UInteger[] {UInteger.valueOf(0L)},
          new LocalizedText(null, ""))
    };
  }

  /** Inputs values in declared order. */
  public static record Inputs(
      @Nullable String transportProfileUri,
      @Nullable UShort numReqWriterGroupIds,
      @Nullable UShort numReqDataSetWriterIds) {
    /**
     * Encodes every declared position.
     *
     * @throws UaException if a value violates its declaration or cannot be encoded.
     */
    public Variant[] toVariants(EncodingContext context) throws UaException {
      return new Variant[] {
        MethodValues.encode(context, transportProfileUri, 0, String.class, -1, new long[] {}, null),
        MethodValues.encode(
            context, numReqWriterGroupIds, 1, UShort.class, -1, new long[] {}, null),
        MethodValues.encode(
            context, numReqDataSetWriterIds, 2, UShort.class, -1, new long[] {}, null)
      };
    }

    /**
     * Decodes the declared positions.
     *
     * @throws UaException if the count, type, rank, dimensions or encoding is invalid.
     */
    public static Inputs fromVariants(EncodingContext context, Variant[] values)
        throws UaException {
      MethodValues.count(values, 3, 3);
      return new Inputs(
          (String) MethodValues.decode(context, values, 0, String.class, -1, new long[] {}, null),
          (UShort) MethodValues.decode(context, values, 1, UShort.class, -1, new long[] {}, null),
          (UShort) MethodValues.decode(context, values, 2, UShort.class, -1, new long[] {}, null));
    }
  }

  /** Outputs values in declared order. */
  public static record Outputs(
      @Nullable Variant defaultPublisherId,
      UShort @Nullable [] writerGroupIds,
      UShort @Nullable [] dataSetWriterIds) {
    /**
     * Encodes every declared position.
     *
     * @throws UaException if a value violates its declaration or cannot be encoded.
     */
    public Variant[] toVariants(EncodingContext context) throws UaException {
      return new Variant[] {
        MethodValues.encode(context, defaultPublisherId, 0, Variant.class, -1, new long[] {}, null),
        MethodValues.encode(context, writerGroupIds, 1, UShort.class, 1, new long[] {0L}, null),
        MethodValues.encode(context, dataSetWriterIds, 2, UShort.class, 1, new long[] {0L}, null)
      };
    }

    /**
     * Decodes the declared positions.
     *
     * @throws UaException if the count, type, rank, dimensions or encoding is invalid.
     */
    public static Outputs fromVariants(EncodingContext context, Variant[] values)
        throws UaException {
      MethodValues.count(values, 3, 3);
      return new Outputs(
          (Variant) MethodValues.decode(context, values, 0, Variant.class, -1, new long[] {}, null),
          (UShort[])
              MethodValues.decode(context, values, 1, UShort.class, 1, new long[] {0L}, null),
          (UShort[])
              MethodValues.decode(context, values, 2, UShort.class, 1, new long[] {0L}, null));
    }

    @Override
    public boolean equals(@Nullable Object other) {
      return other instanceof Outputs value
          && Arrays.deepEquals(
              new Object[] {defaultPublisherId, writerGroupIds, dataSetWriterIds},
              new Object[] {
                value.defaultPublisherId, value.writerGroupIds, value.dataSetWriterIds
              });
    }

    @Override
    public int hashCode() {
      return Arrays.deepHashCode(
          new Object[] {defaultPublisherId, writerGroupIds, dataSetWriterIds});
    }

    @Override
    public String toString() {
      return "Outputs"
          + Arrays.deepToString(
              new Object[] {defaultPublisherId, writerGroupIds, dataSetWriterIds});
    }
  }
}
