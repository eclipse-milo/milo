...
public class ExclusiveLimitAlarmTypeNode extends LimitAlarmTypeNode
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:ActiveState (declaration i=9398,"
                                    + " owner i=9341) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                              ...
                              new RelativePath(
                                  new RelativePathElement[] {
                                    new RelativePathElement(
                                        ...
                                        false,
                                        true,
                                        new QualifiedName(namespaceIndex, "ActiveState"))
                                  }));
                      return client
                                ...
                                if (results == null
                                  ...
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          ...
                                          "http://opcfoundation.org/UA/:ActiveState (declaration"
                                              + " i=9398, owner i=9341) on "
                                              + getNodeId()));
                                }
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:ActiveState"
                                                        + " (declaration i=9398, owner i=9341) on "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
                                            return CompletableFuture.failedFuture(
  ...
  public CompletableFuture<? extends ExclusiveLimitStateMachineTypeNode> getLimitStateNodeAsync() {
    CompletableFuture<ExclusiveLimitStateMachineTypeNode> result = new CompletableFuture<>();
    try {
      CompletableFuture<NodeId> lookup = CompletableFuture.completedFuture(getNodeId());
      CompletableFuture<UaNode> hop0 =
          lookup.thenCompose(
                    ...
                    ignored -> {
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                StatusCodes.Bad_NodeIdInvalid,
                                "http://opcfoundation.org/UA/:LimitState (declaration i=9455, owner"
                                    + " i=9341) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                              ...
                              new RelativePath(
                                  new RelativePathElement[] {
                                    new RelativePathElement(
                                        ...
                                        false,
                                        true,
                                        new QualifiedName(namespaceIndex, "LimitState"))
                                  }));
                      return client
                          .translateBrowsePathsAsync(List.of(browsePath))
                          .thenCompose(
                              response -> {
                                ...
                                if (results == null
                                  ...
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          StatusCodes.Bad_UnexpectedError,
                                          "http://opcfoundation.org/UA/:LimitState (declaration"
                                              + " i=9455, owner i=9341) on "
                                              + getNodeId()));
                                }
                                var operation = results[0];
                                if (operation.getStatusCode().getValue()
                                                ...
                                                namespaceTable -> {
                                                  ...
                                                  if (resolvedTarget.isEmpty()) {
                                                    return CompletableFuture.failedFuture(
                                                        new UaException(
                                                            StatusCodes.Bad_NodeIdInvalid,
                                                            "http://opcfoundation.org/UA/:LimitState"
                                                                + " (declaration i=9455, owner"
                                                                + " i=9341) on "
                                                                + getNodeId()));
                                                  }
                                                  return CompletableFuture.completedFuture(
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:LimitState"
                                                        + " (declaration i=9455, owner i=9341) on "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
                                            return CompletableFuture.failedFuture(
      ...
      hop0.whenComplete(
          (node, failure) -> {
            if (failure != null) {
              result.completeExceptionally(failure);
            } else if (node != null && !(node instanceof ExclusiveLimitStateMachineTypeNode)) {
              result.completeExceptionally(
                  new UaException(
                      StatusCodes.Bad_TypeMismatch,
                      "http://opcfoundation.org/UA/:LimitState (declaration i=9455, owner"
              ...
              result.complete((ExclusiveLimitStateMachineTypeNode) node);
            }
          });
    } catch (Exception e) {
      result.completeExceptionally(e);
