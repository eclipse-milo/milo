...
public class AuditAddReferencesEventTypeNode extends AuditNodeManagementEventTypeNode
   ...
   */
  public static ClientViews createViews(AuditAddReferencesEventTypeNode node) {
    Objects.requireNonNull(node, "node");
    return ClientViews.forNode(node.client, node);
  }

  @Override
  public @Nullable AddReferencesItem @Nullable [] getReferencesToAdd() throws UaException {
    PropertyTypeNode node = getReferencesToAddNode();
    if (node == null) {
      throw new UaException(
          StatusCodes.Bad_NotFound,
          "http://opcfoundation.org/UA/:ReferencesToAdd (declaration i=2096, owner i=2095)"
              + " on "
              + getNodeId());
    }
  ...
  @Override
  public void setReferencesToAdd(@Nullable AddReferencesItem @Nullable [] value)
      throws UaException {
    PropertyTypeNode node = getReferencesToAddNode();
    if (node == null) {
      throw new UaException(
    ...
    node.setValue(
        new Variant(encodeValue(value, AddReferencesItem.class, ValueRanks.OneDimension)));
  }
            ...
            node -> {
              if (node == null) {
                throw new CompletionException(
                    new UaException(
                        StatusCodes.Bad_NotFound,
                        "http://opcfoundation.org/UA/:ReferencesToAdd (declaration i=2096, owner"
                            + " i=2095) on "
                            + getNodeId()));
              }
        ...
        .thenApply(
            v -> {
              ...
              try {
                return (AddReferencesItem[])
                    decodeValue(
                        v.getValue().getValue(), AddReferencesItem.class, ValueRanks.OneDimension);
  ...
  public CompletableFuture<StatusCode> writeReferencesToAddAsync(
      @Nullable AddReferencesItem @Nullable [] referencesToAdd) {
    return getReferencesToAddNodeAsync()
        .thenCompose(
            node -> {
              if (node == null) {
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:ReferencesToAdd (declaration i=2096,"
                                    + " owner i=2095) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                              ...
                              new RelativePath(
                                  new RelativePathElement[] {
                                    new RelativePathElement(
                                        ...
                                        false,
                                        true,
                                        new QualifiedName(namespaceIndex, "ReferencesToAdd"))
                                  }));
                      return client
                                  ...
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          ...
                                          "http://opcfoundation.org/UA/:ReferencesToAdd"
                                              + " (declaration i=2096, owner i=2095) on "
                                              + getNodeId()));
                                }
                                ...
                                if (!operation.getStatusCode().isGood()) {
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          ...
                                          "http://opcfoundation.org/UA/:ReferencesToAdd"
                                              + " (declaration i=2096, owner i=2095)"));
                                }
