                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:ResourceUri (declaration i=15156,"
                                    + " owner i=15155) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:ResourceUri"
                                                        + " (declaration i=15156, owner i=15155) on"
                                                        + " "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
                                  ...
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          ...
                                          "http://opcfoundation.org/UA/:AuthenticationProfileUri"
                                              + " (declaration i=15178, owner i=15155) on "
                                              + getNodeId()));
                                }
                                var operation = results[0];
                                if (operation.getStatusCode().getValue()
                                ...
                                if (!operation.getStatusCode().isGood()) {
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          operation.getStatusCode(),
                                          "http://opcfoundation.org/UA/:AuthenticationProfileUri"
                                              + " (declaration i=15178, owner i=15155)"));
                                }
                                var targets = operation.getTargets();
                                if (targets == null || targets.length == 0) {
                                  return CompletableFuture.failedFuture(
          ...
          (node, failure) -> {
            if (failure != null) {
              ...
              result.completeExceptionally(
                  new UaException(
                      StatusCodes.Bad_TypeMismatch,
                      "http://opcfoundation.org/UA/:AuthenticationProfileUri (declaration i=15178,"
                          + " owner i=15155)"));
            } else {
              result.complete((PropertyTypeNode) node);
            }
