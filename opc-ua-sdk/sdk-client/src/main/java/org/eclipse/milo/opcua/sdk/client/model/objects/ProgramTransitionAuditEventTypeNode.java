...
public class ProgramTransitionAuditEventTypeNode extends AuditUpdateStateEventTypeNode
   ...
   */
  public static ClientViews createViews(ProgramTransitionAuditEventTypeNode node) {
  ...
  public @Nullable LocalizedText getTransition() throws UaException {
    ...
    if (node == null) {
      throw new UaException(
          ...
          "http://opcfoundation.org/UA/:Transition (declaration i=3825, owner i=3806)"
              + " on "
              + getNodeId());
    }
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:Transition (declaration i=3825, owner"
                                    + " i=3806) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:Transition"
                                                        + " (declaration i=3825, owner i=3806) on "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
                                            return CompletableFuture.failedFuture(
