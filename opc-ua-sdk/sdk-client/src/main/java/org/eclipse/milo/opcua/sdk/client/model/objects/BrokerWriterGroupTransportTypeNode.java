...
public class BrokerWriterGroupTransportTypeNode extends WriterGroupTransportTypeNode
   ...
   */
  public static ClientViews createViews(BrokerWriterGroupTransportTypeNode node) {
            ...
            node -> {
              if (node == null) {
                throw new CompletionException(
                    new UaException(
                        StatusCodes.Bad_NotFound,
                        "http://opcfoundation.org/UA/:QueueName (declaration i=21137, owner"
                            + " i=21136) on "
                            + getNodeId()));
              }
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:QueueName"
                                                        + " (declaration i=21137, owner i=21136) on"
                                                        + " "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
            ...
            node -> {
              if (node == null) {
                throw new CompletionException(
                    new UaException(
                        StatusCodes.Bad_NotFound,
                        "http://opcfoundation.org/UA/:ResourceUri (declaration i=15246, owner"
                            + " i=21136) on "
                            + getNodeId()));
              }
              return node.readAttributeAsync(AttributeId.Value);
                    ...
                    ignored -> {
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                StatusCodes.Bad_NodeIdInvalid,
                                "http://opcfoundation.org/UA/:ResourceUri (declaration i=15246,"
                                    + " owner i=21136) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:ResourceUri"
                                                        + " (declaration i=15246, owner i=21136) on"
                                                        + " "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
                                ...
                                if (!operation.getStatusCode().isGood()) {
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          operation.getStatusCode(),
                                          "http://opcfoundation.org/UA/:AuthenticationProfileUri"
                                              + " (declaration i=15247, owner i=21136)"));
                                }
                                var targets = operation.getTargets();
                                if (targets == null || targets.length == 0) {
                                  return CompletableFuture.failedFuture(
          ...
          (node, failure) -> {
            if (failure != null) {
              ...
              result.completeExceptionally(
                  new UaException(
                      StatusCodes.Bad_TypeMismatch,
                      "http://opcfoundation.org/UA/:AuthenticationProfileUri (declaration i=15247,"
                          + " owner i=21136)"));
            } else {
              result.complete((PropertyTypeNode) node);
            }
                                ...
                                if (!operation.getStatusCode().isGood()) {
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          operation.getStatusCode(),
                                          "http://opcfoundation.org/UA/:RequestedDeliveryGuarantee"
                                              + " (declaration i=15249, owner i=21136)"));
                                }
                                var targets = operation.getTargets();
                                if (targets == null || targets.length == 0) {
                                  return CompletableFuture.failedFuture(
