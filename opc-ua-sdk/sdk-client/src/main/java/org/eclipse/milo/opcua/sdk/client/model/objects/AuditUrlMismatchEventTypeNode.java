...
public class AuditUrlMismatchEventTypeNode extends AuditCreateSessionEventTypeNode
   ...
   */
  public static ClientViews createViews(AuditUrlMismatchEventTypeNode node) {
  ...
  public @Nullable String getEndpointUrl() throws UaException {
    ...
    if (node == null) {
      throw new UaException(
          ...
          "http://opcfoundation.org/UA/:EndpointUrl (declaration i=2749, owner i=2748)"
              + " on "
              + getNodeId());
    }
            ...
            node -> {
              if (node == null) {
                throw new CompletionException(
                    new UaException(
                        StatusCodes.Bad_NotFound,
                        "http://opcfoundation.org/UA/:EndpointUrl (declaration i=2749, owner"
                            + " i=2748) on "
                            + getNodeId()));
              }
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:EndpointUrl (declaration i=2749,"
                                    + " owner i=2748) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:EndpointUrl"
                                                        + " (declaration i=2749, owner i=2748) on "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
                                            return CompletableFuture.failedFuture(
