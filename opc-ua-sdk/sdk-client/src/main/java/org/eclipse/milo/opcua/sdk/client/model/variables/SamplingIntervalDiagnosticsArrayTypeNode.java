...
public class SamplingIntervalDiagnosticsArrayTypeNode extends BaseDataVariableTypeNode
  ...
  public CompletableFuture<? extends SamplingIntervalDiagnosticsTypeNode>
    ...
    CompletableFuture<SamplingIntervalDiagnosticsTypeNode> result = new CompletableFuture<>();
    try {
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:SamplingIntervalDiagnostics"
                                    + " (declaration i=12779, owner i=2164) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                          new BrowsePath(
                              ...
                              new RelativePath(
                                  new RelativePathElement[] {
                                    new RelativePathElement(
                                        ...
                                        true,
                                        new QualifiedName(
                                            namespaceIndex, "SamplingIntervalDiagnostics"))
                                  }));
                      return client
                                ...
                                if (!operation.getStatusCode().isGood()) {
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          ...
                                          "http://opcfoundation.org/UA/:SamplingIntervalDiagnostics"
                                              + " (declaration i=12779, owner i=2164)"));
                                }
      ...
      hop0.whenComplete(
          (node, failure) -> {
            ...
            } else if (node != null && !(node instanceof SamplingIntervalDiagnosticsTypeNode)) {
              result.completeExceptionally(
                  new UaException(
                      StatusCodes.Bad_TypeMismatch,
                      "http://opcfoundation.org/UA/:SamplingIntervalDiagnostics (declaration"
                          + " i=12779, owner i=2164)"));
            } else {
              result.complete((SamplingIntervalDiagnosticsTypeNode) node);
            }
          });
    } catch (Exception e) {
