...
public class StateTypeNode extends BaseObjectTypeNode implements StateType {
   ...
   */
  public static ClientViews createViews(StateTypeNode node) {
    Objects.requireNonNull(node, "node");
    return ClientViews.forNode(node.client, node);
  }

  @Override
  public @Nullable UInteger getStateNumber() throws UaException {
    PropertyTypeNode node = getStateNumberNode();
    if (node == null) {
      throw new UaException(
          StatusCodes.Bad_NotFound,
          "http://opcfoundation.org/UA/:StateNumber (declaration i=2308, owner i=2307)"
              + " on "
              + getNodeId());
    }
  ...
  @Override
  public void setStateNumber(@Nullable UInteger value) throws UaException {
    PropertyTypeNode node = getStateNumberNode();
    if (node == null) {
      throw new UaException(
          StatusCodes.Bad_NotFound,
            ...
            node -> {
              if (node == null) {
                throw new CompletionException(
                    new UaException(
                        StatusCodes.Bad_NotFound,
                        "http://opcfoundation.org/UA/:StateNumber (declaration i=2308, owner"
                            + " i=2307) on "
                            + getNodeId()));
              }
  ...
  @Override
  public CompletableFuture<StatusCode> writeStateNumberAsync(@Nullable UInteger stateNumber) {
    return getStateNumberNodeAsync()
        .thenCompose(
            node -> {
              if (node == null) {
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:StateNumber (declaration i=2308,"
                                    + " owner i=2307) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                              ...
                              new RelativePath(
                                  new RelativePathElement[] {
                                    new RelativePathElement(
                                        ...
                                        false,
                                        true,
                                        new QualifiedName(namespaceIndex, "StateNumber"))
                                  }));
                      return client
                                ...
                                if (results == null
                                  ...
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          ...
                                          "http://opcfoundation.org/UA/:StateNumber (declaration"
                                              + " i=2308, owner i=2307) on "
                                              + getNodeId()));
                                }
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:StateNumber"
                                                        + " (declaration i=2308, owner i=2307) on "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
                                            return CompletableFuture.failedFuture(
