...
public class AuditHistoryDeleteEventTypeNode extends AuditHistoryUpdateEventTypeNode
   ...
   */
  public static ClientViews createViews(AuditHistoryDeleteEventTypeNode node) {
  ...
  public @Nullable NodeId getUpdatedNode() throws UaException {
    ...
    if (node == null) {
      throw new UaException(
          ...
          "http://opcfoundation.org/UA/:UpdatedNode (declaration i=3027, owner i=3012)"
              + " on "
              + getNodeId());
    }
            ...
            node -> {
              if (node == null) {
                throw new CompletionException(
                    new UaException(
                        StatusCodes.Bad_NotFound,
                        "http://opcfoundation.org/UA/:UpdatedNode (declaration i=3027, owner"
                            + " i=3012) on "
                            + getNodeId()));
              }
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:UpdatedNode (declaration i=3027,"
                                    + " owner i=3012) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:UpdatedNode"
                                                        + " (declaration i=3027, owner i=3012) on "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
                                            return CompletableFuture.failedFuture(
