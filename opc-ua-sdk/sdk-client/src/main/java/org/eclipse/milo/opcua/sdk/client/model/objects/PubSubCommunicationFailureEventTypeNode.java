...
public class PubSubCommunicationFailureEventTypeNode extends PubSubStatusEventTypeNode
   ...
   */
  public static ClientViews createViews(PubSubCommunicationFailureEventTypeNode node) {
    Objects.requireNonNull(node, "node");
    return ClientViews.forNode(node.client, node);
  }

  @Override
  public @Nullable StatusCode getError() throws UaException {
    PropertyTypeNode node = getErrorNode();
    if (node == null) {
      throw new UaException(
          StatusCodes.Bad_NotFound,
          "http://opcfoundation.org/UA/:Error (declaration i=15576, owner i=15563)"
              + " on "
              + getNodeId());
    }
  ...
  @Override
  public void setError(@Nullable StatusCode value) throws UaException {
    PropertyTypeNode node = getErrorNode();
    if (node == null) {
      throw new UaException(
          StatusCodes.Bad_NotFound,
  ...
  @Override
  public CompletableFuture<StatusCode> writeErrorAsync(@Nullable StatusCode error) {
    return getErrorNodeAsync()
        .thenCompose(
            node -> {
              if (node == null) {
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:Error (declaration i=15576, owner"
                                    + " i=15563) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                              ...
                              new RelativePath(
                                  new RelativePathElement[] {
                                    new RelativePathElement(
                                        ...
                                        false,
                                        true,
                                        new QualifiedName(namespaceIndex, "Error"))
                                  }));
                      return client
                                ...
                                if (results == null
                                  ...
                                  return CompletableFuture.failedFuture(
                                      new UaException(
                                          ...
                                          "http://opcfoundation.org/UA/:Error (declaration i=15576,"
                                              + " owner i=15563) on "
                                              + getNodeId()));
                                }
                                ...
                                for (var target : targets) {
                                  if (target == null
                                    ...
                                    return CompletableFuture.failedFuture(
                                        new UaException(
                                            StatusCodes.Bad_UnexpectedError,
                                            "http://opcfoundation.org/UA/:Error (declaration"
                                                + " i=15576, owner i=15563) on "
                                                + getNodeId()));
                                  }
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:Error"
                                                        + " (declaration i=15576, owner i=15563) on"
                                                        + " "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
