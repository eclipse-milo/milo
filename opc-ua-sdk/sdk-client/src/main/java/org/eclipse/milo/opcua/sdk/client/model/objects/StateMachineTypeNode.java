...
public class StateMachineTypeNode extends BaseObjectTypeNode implements StateMachineType {
   ...
   */
  public static ClientViews createViews(StateMachineTypeNode node) {
            ...
            node -> {
              if (node == null) {
                throw new CompletionException(
                    new UaException(
                        StatusCodes.Bad_NotFound,
                        "http://opcfoundation.org/UA/:CurrentState (declaration i=2769, owner"
                            + " i=2299) on "
                            + getNodeId()));
              }
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                ...
                                "http://opcfoundation.org/UA/:CurrentState (declaration i=2769,"
                                    + " owner i=2299) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                                          ...
                                          if (unique.stream().anyMatch(NodeId::isNull)) {
                                            return CompletableFuture.failedFuture(
                                                new UaException(
                                                    StatusCodes.Bad_NodeIdInvalid,
                                                    "http://opcfoundation.org/UA/:CurrentState"
                                                        + " (declaration i=2769, owner i=2299) on "
                                                        + getNodeId()));
                                          }
                                          if (unique.size() != 1) {
                                            return CompletableFuture.failedFuture(
            ...
            node -> {
              if (node == null) {
                throw new CompletionException(
                    new UaException(
                        StatusCodes.Bad_NotFound,
                        "http://opcfoundation.org/UA/:LastTransition (declaration i=2770, owner"
                            + " i=2299) on "
                            + getNodeId()));
              }
              return node.readAttributeAsync(AttributeId.Value);
                    ...
                    ignored -> {
                      ...
                      if (namespaceIndex == null || referenceId.isEmpty()) {
                        return CompletableFuture.failedFuture(
                            new UaException(
                                StatusCodes.Bad_NodeIdInvalid,
                                "http://opcfoundation.org/UA/:LastTransition (declaration i=2770,"
                                    + " owner i=2299) on "
                                    + getNodeId()));
                      }
                      var browsePath =
                                    ...
                                    return CompletableFuture.failedFuture(
                                        new UaException(
                                            ...
                                            "http://opcfoundation.org/UA/:LastTransition"
                                                + " (declaration i=2770, owner i=2299) on "
                                                + getNodeId()));
                                  }
                                  if (!target.getTargetId().isLocal()) {
                                    return CompletableFuture.failedFuture(
