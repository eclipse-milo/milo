 ...
 * §9.1.10).
 *
 * <p>Check order: session &rarr; arguments (none) &rarr; authorization via the effective {@link
 * PubSubMethodAuthorizer} (whose code is surfaced verbatim) &rarr; existence &rarr; state rule
 * &rarr; work. Remote-configuration Methods have no spec channel-security minimum, so no
 * channel-mode gate exists; a session-less invocation is rejected with {@code
 ...
 *
 * <p>The handlers call {@link PubSubService#enable}/{@link PubSubService#disable} directly and do
 * no diagnostics counter work — the engine attributes the METHOD cause to those calls regardless of
 * caller. Enable/Disable are <b>not</b> configuration mutations: they never save to a {@link
 * PubSubConfigurationStore} and they do not run through {@link ManagedPubSubService}'s post-apply
 * hooks — never wire a persistence hook to them.
 *
 * <p>Handles are resolved per invocation through the {@code handleLookup} captured at mint time
 * (name components, never parsed paths). An empty lookup answers {@code Bad_NotFound}; this is
 * reachable only in the window between an engine apply and the fragment rebuild that deletes the
...
final class PubSubStatusMethods {
  ...
  /**
   * The shared session &rarr; authorization &rarr; existence prefix of the check order.
   *
   * @return the component's current handle.
   * @throws UaException {@code Bad_UserAccessDenied} for a session-less invocation, the
