...
/**
 * Definition of a single field in a {@link PublishedDataSetConfig}.
 *
 ...
 * called. The {@code promoted} flag maps to the {@code FieldMetaData} field flags. The {@code
 * valueRank} and {@code arrayDimensions} values exist for metadata fidelity and default to scalar
 * ({@code -1}) with no dimensions.
 */
public final class FieldDefinition {
  ...
  private FieldDefinition(Builder builder, FieldAddress source, UUID dataSetFieldId) {
  ...
  /**
   * Get the source address of this field.
   *
   * @return the source address; defaults to a {@link KeyFieldAddress} keyed by the field name.
   */
  public FieldAddress getSource() {
    return source;
  }
  ...
  public boolean equals(@Nullable Object obj) {
    ...
    if (!(obj instanceof FieldDefinition that)) {
      return false;
    }
  ...
  public int hashCode() {
    int result =
        Objects.hash(name, source, dataType, dataSetFieldId, promoted, valueRank, properties);
  ...
  public String toString() {
    return "FieldDefinition{name='%s', source=%s, dataType=%s, dataSetFieldId=%s, promoted=%s, valueRank=%s, arrayDimensions=%s, properties=%s}"
        .formatted(
            name,
            source,
            dataType,
  ...
  /** A builder of {@link FieldDefinition} instances. */
  public static final class Builder {

    private final String name;
    private @Nullable FieldAddress source;
    ...
    /**
     * Set the source address of the field.
     *
     * <p>If not set, the source defaults to a {@link KeyFieldAddress} keyed by the field name when
     * {@link #build()} is called.
     *
     * @param source the source address.
     * @return this {@link Builder}.
     */
    public Builder source(FieldAddress source) {
      this.source = source;
      return this;
    }
    ...
    /**
     * Build a {@link FieldDefinition} from the configured values.
     *
     * @return a new {@link FieldDefinition}.
     * @throws PubSubConfigValidationException if the field name is empty.
     */
    public FieldDefinition build() {
      if (name.isEmpty()) {
        throw new PubSubConfigValidationException("FieldDefinition: name must not be empty");
      }

      FieldAddress source = this.source != null ? this.source : new KeyFieldAddress(name);
      UUID dataSetFieldId = this.dataSetFieldId != null ? this.dataSetFieldId : UUID.randomUUID();

      return new FieldDefinition(this, source, dataSetFieldId);
    }
  }
}
