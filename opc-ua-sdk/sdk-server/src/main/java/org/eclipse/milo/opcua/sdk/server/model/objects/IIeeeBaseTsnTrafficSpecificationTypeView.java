 ...
 * node. Obtain this view from a ServerViews context using {@link #TYPE}; the token itself does not
 * prove membership. Access after context close fails with IllegalStateException.
 */
@NullMarked
public final class IIeeeBaseTsnTrafficSpecificationTypeView extends ServerObjectView
    implements IIeeeBaseTsnTrafficSpecificationType {
  /** Selected UA/Java contract and context-owned factory. */
  public static final ServerViewType<IIeeeBaseTsnTrafficSpecificationType> TYPE =
      ServerViewType.of(
  ...
  private IIeeeBaseTsnTrafficSpecificationTypeView(ServerViews views, UaObjectNode node) {
    super(views, node);
  }
        ...
        ((Supplier<UaVariableNode>)
                () -> {
                  UaNode parent = this.node;
                  {
                    ...
                    var browseName = new QualifiedName(namespaceIndex, "MaxIntervalFrames");
        ...
        ((Supplier<UaVariableNode>)
                () -> {
                  UaNode parent = this.node;
                  {
                    ...
                    var browseName = new QualifiedName(namespaceIndex, "MaxFrameSize");
                    var matches = new LinkedHashMap<NodeId, UaNode>();
                    for (var reference : parent.getReferences()) {
                      if (!reference.isForward()) {
                        continue;
        ...
        ((Supplier<UaVariableNode>)
                () -> {
                  UaNode parent = this.node;
                  {
                    ...
                    var browseName = new QualifiedName(namespaceIndex, "Interval");
                    var matches = new LinkedHashMap<NodeId, UaNode>();
                    for (var reference : parent.getReferences()) {
                      if (!reference.isForward()) {
                        continue;
      ...
      try {
        rawValue =
            ExtensionObject.decodeValue(
                node.getNodeContext().getServer().getStaticEncodingContext(), rawValue);
