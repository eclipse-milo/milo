...
public class SemanticChangeEventTypeNode extends BaseEventTypeNode
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    ...
                    + " while resolving http://opcfoundation.org/UA/:Changes (declaration i=2739,"
                    + " owner i=2738) on "
                    + getNodeId());
          }
  ...
  public @Nullable SemanticChangeStructureDataType @Nullable [] getChanges() {
    ...
    SemanticChangeStructureDataType[] typed = new SemanticChangeStructureDataType[elements.length];
    for (int i = 0; i < elements.length; i++) {
      if (elements[i] != null && !(elements[i] instanceof SemanticChangeStructureDataType)) {
        throw new UaRuntimeException(
            StatusCodes.Bad_TypeMismatch,
            "http://opcfoundation.org/UA/:Changes (declaration i=2739, owner i=2738)");
      }
      typed[i] = (SemanticChangeStructureDataType) elements[i];
    }
    return typed;
  }
