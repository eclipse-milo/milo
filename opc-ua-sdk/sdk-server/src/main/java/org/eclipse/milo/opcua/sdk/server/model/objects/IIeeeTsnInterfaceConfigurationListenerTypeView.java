 ...
 * backing node. Obtain this view from a ServerViews context using {@link #TYPE}; the token itself
 * does not prove membership. Access after context close fails with IllegalStateException.
 */
@NullMarked
public final class IIeeeTsnInterfaceConfigurationListenerTypeView extends ServerObjectView
    implements IIeeeTsnInterfaceConfigurationListenerType {
  /** Selected UA/Java contract and context-owned factory. */
  public static final ServerViewType<IIeeeTsnInterfaceConfigurationListenerType> TYPE =
      ServerViewType.of(
  ...
  private IIeeeTsnInterfaceConfigurationListenerTypeView(ServerViews views, UaObjectNode node) {
    super(views, node);
  }
        ...
        ((Supplier<UaVariableNode>)
                () -> {
                  UaNode parent = this.node;
                  {
                    ...
                    var browseName = new QualifiedName(namespaceIndex, "MacAddress");
        ...
        ((Supplier<@Nullable UaVariableNode>)
                () -> {
                  UaNode parent = this.node;
                  {
                    ...
                    var browseName = new QualifiedName(namespaceIndex, "InterfaceName");
                    var matches = new LinkedHashMap<NodeId, UaNode>();
                    for (var reference : parent.getReferences()) {
                      if (!reference.isForward()) {
                        continue;
        ...
        ((Supplier<@Nullable UaVariableNode>)
                () -> {
                  UaNode parent = this.node;
                  {
                    ...
                    var browseName = new QualifiedName(namespaceIndex, "ReceiveOffset");
                    var matches = new LinkedHashMap<NodeId, UaNode>();
                    for (var reference : parent.getReferences()) {
                      if (!reference.isForward()) {
                        continue;
  ...
  @Override
  public @Nullable BaseDataVariableType getInterfaceNameNode() {
    this.views.checkOpen();
    views.checkOpen();
  ...
  @Override
  public @Nullable BaseDataVariableType getReceiveOffsetNode() {
    this.views.checkOpen();
    views.checkOpen();
