...
public class NDimensionArrayItemTypeNode extends ArrayItemTypeNode
  ...
  public PropertyTypeNode getAxisDefinitionNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "AxisDefinition");
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    ...
                    + " while resolving http://opcfoundation.org/UA/:AxisDefinition (declaration"
  ...
  public @Nullable AxisInformation @Nullable [] getAxisDefinition() {
    var node = getAxisDefinitionNode();
    if (node == null) {
      throw new UaRuntimeException(
          StatusCodes.Bad_NotFound,
          "http://opcfoundation.org/UA/:AxisDefinition (declaration i=12076, owner i=12068)"
    ...
    AxisInformation[] typed = new AxisInformation[elements.length];
    for (int i = 0; i < elements.length; i++) {
      if (elements[i] != null && !(elements[i] instanceof AxisInformation)) {
        throw new UaRuntimeException(
            StatusCodes.Bad_TypeMismatch,
            "http://opcfoundation.org/UA/:AxisDefinition (declaration i=12076, owner i=12068)");
      }
      typed[i] = (AxisInformation) elements[i];
    }
    return typed;
  }
