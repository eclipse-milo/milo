...
public class ProgressEventTypeNode extends BaseEventTypeNode implements ProgressEventType {
  ...
  public PropertyTypeNode getContextNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "Context");
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    ...
                    + " while resolving http://opcfoundation.org/UA/:Context (declaration i=12502,"
                    + " owner i=11436) on "
                    + getNodeId());
          }
  ...
  public PropertyTypeNode getProgressNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "Progress");
      var matches = new LinkedHashMap<NodeId, UaNode>();
      for (var reference : parent.getReferences()) {
        if (!reference.isForward()) {
          continue;
        ...
        if (!reference.getReferenceTypeId().equals(referenceId.orElseThrow())) {
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    + reference.getReferenceTypeId()
                    + " while resolving http://opcfoundation.org/UA/:Progress (declaration i=12503,"
                    + " owner i=11436) on "
                    + getNodeId());
          }
          if (!(referenceTypeTree.isSubtypeOf(
