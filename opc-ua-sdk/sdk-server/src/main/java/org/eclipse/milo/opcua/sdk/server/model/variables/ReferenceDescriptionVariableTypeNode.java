...
public class ReferenceDescriptionVariableTypeNode extends BaseDataVariableTypeNode
  ...
  public @Nullable PropertyTypeNode getReferenceRefinementNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "ReferenceRefinement");
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    ...
                    + " while resolving http://opcfoundation.org/UA/:ReferenceRefinement"
  ...
  public @Nullable ReferenceListEntryDataType @Nullable [] getReferenceRefinement() {
    var node = getReferenceRefinementNode();
    if (node == null) {
      throw new UaRuntimeException(
    ...
    ReferenceListEntryDataType[] typed = new ReferenceListEntryDataType[elements.length];
    for (int i = 0; i < elements.length; i++) {
      if (elements[i] != null && !(elements[i] instanceof ReferenceListEntryDataType)) {
        throw new UaRuntimeException(
            StatusCodes.Bad_TypeMismatch,
            "http://opcfoundation.org/UA/:ReferenceRefinement (declaration i=32658, owner"
                + " i=32657)");
      }
      typed[i] = (ReferenceListEntryDataType) elements[i];
    }
    return typed;
  }
