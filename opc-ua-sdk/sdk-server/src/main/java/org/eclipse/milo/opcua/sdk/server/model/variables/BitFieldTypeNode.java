...
public class BitFieldTypeNode extends BaseDataVariableTypeNode implements BitFieldType {
  ...
  @Override
  public PropertyTypeNode getBitFieldsDefinitionsNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "BitFieldsDefinitions");
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    ...
                    + " while resolving http://opcfoundation.org/UA/:BitFieldsDefinitions"
                    + " (declaration i=32432, owner i=32431) on "
                    + getNodeId());
          }
  ...
  @Override
  public @Nullable BitFieldDefinition @Nullable [] getBitFieldsDefinitions() {
    var node = getBitFieldsDefinitionsNode();
    if (node == null) {
      throw new UaRuntimeException(
          StatusCodes.Bad_NotFound,
          "http://opcfoundation.org/UA/:BitFieldsDefinitions (declaration i=32432, owner i=32431)"
    ...
    BitFieldDefinition[] typed = new BitFieldDefinition[elements.length];
    for (int i = 0; i < elements.length; i++) {
      if (elements[i] != null && !(elements[i] instanceof BitFieldDefinition)) {
        throw new UaRuntimeException(
            StatusCodes.Bad_TypeMismatch,
            "http://opcfoundation.org/UA/:BitFieldsDefinitions (declaration i=32432, owner"
                + " i=32431)");
      }
      typed[i] = (BitFieldDefinition) elements[i];
    }
    return typed;
  }
