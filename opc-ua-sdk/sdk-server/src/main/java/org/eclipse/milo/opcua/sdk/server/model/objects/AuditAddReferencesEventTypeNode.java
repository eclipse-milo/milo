...
public class AuditAddReferencesEventTypeNode extends AuditNodeManagementEventTypeNode
  ...
  public PropertyTypeNode getReferencesToAddNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "ReferencesToAdd");
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    ...
                    + " while resolving http://opcfoundation.org/UA/:ReferencesToAdd (declaration"
                    + " i=2096, owner i=2095) on "
                    + getNodeId());
          }
  ...
  @Override
  public @Nullable AddReferencesItem @Nullable [] getReferencesToAdd() {
    var node = getReferencesToAddNode();
    if (node == null) {
      throw new UaRuntimeException(
          StatusCodes.Bad_NotFound,
          "http://opcfoundation.org/UA/:ReferencesToAdd (declaration i=2096, owner i=2095)"
    ...
    AddReferencesItem[] typed = new AddReferencesItem[elements.length];
    for (int i = 0; i < elements.length; i++) {
      if (elements[i] != null && !(elements[i] instanceof AddReferencesItem)) {
        throw new UaRuntimeException(
            StatusCodes.Bad_TypeMismatch,
            "http://opcfoundation.org/UA/:ReferencesToAdd (declaration i=2096, owner i=2095)");
      }
      typed[i] = (AddReferencesItem) elements[i];
    }
    return typed;
  }
