...
public class GeneralModelChangeEventTypeNode extends BaseModelChangeEventTypeNode
  ...
  public PropertyTypeNode getChangesNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "Changes");
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    ...
                    + " while resolving http://opcfoundation.org/UA/:Changes (declaration i=2134,"
                    + " owner i=2133) on "
                    + getNodeId());
          }
  ...
  public @Nullable ModelChangeStructureDataType @Nullable [] getChanges() {
    ...
    ModelChangeStructureDataType[] typed = new ModelChangeStructureDataType[elements.length];
    for (int i = 0; i < elements.length; i++) {
      if (elements[i] != null && !(elements[i] instanceof ModelChangeStructureDataType)) {
        throw new UaRuntimeException(
            StatusCodes.Bad_TypeMismatch,
            "http://opcfoundation.org/UA/:Changes (declaration i=2134, owner i=2133)");
      }
      typed[i] = (ModelChangeStructureDataType) elements[i];
    }
    return typed;
  }
