...
public class PubSubTransportLimitsExceedEventTypeNode extends PubSubStatusEventTypeNode
  ...
  public PropertyTypeNode getActualNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "Actual");
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    ...
                    + " while resolving http://opcfoundation.org/UA/:Actual (declaration i=15561,"
                    + " owner i=15548) on "
                    + getNodeId());
          }
  ...
  public PropertyTypeNode getMaximumNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "Maximum");
      var matches = new LinkedHashMap<NodeId, UaNode>();
      for (var reference : parent.getReferences()) {
        if (!reference.isForward()) {
          continue;
        ...
        if (!reference.getReferenceTypeId().equals(referenceId.orElseThrow())) {
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    + reference.getReferenceTypeId()
                    + " while resolving http://opcfoundation.org/UA/:Maximum (declaration i=15562,"
                    + " owner i=15548) on "
                    + getNodeId());
          }
          if (!(referenceTypeTree.isSubtypeOf(
