...
public class IIeeeTsnVlanTagTypeNode extends BaseInterfaceTypeNode implements IIeeeTsnVlanTagType {
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    ...
                    + " while resolving http://opcfoundation.org/UA/:VlanId (declaration i=24203,"
                    + " owner i=24202) on "
                    + getNodeId());
          }
        ...
        if (!reference.getReferenceTypeId().equals(referenceId.orElseThrow())) {
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    + reference.getReferenceTypeId()
                    + " while resolving http://opcfoundation.org/UA/:PriorityCodePoint (declaration"
  ...
  public @Nullable UByte getPriorityCodePoint() {
    var node = getPriorityCodePointNode();
    if (node == null) {
      throw new UaRuntimeException(
          StatusCodes.Bad_NotFound,
          "http://opcfoundation.org/UA/:PriorityCodePoint (declaration i=24204, owner i=24202)"
