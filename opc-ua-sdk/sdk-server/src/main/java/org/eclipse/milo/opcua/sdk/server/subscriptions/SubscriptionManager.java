...
import org.eclipse.milo.opcua.sdk.server.items.*;
...
import org.eclipse.milo.opcua.sdk.server.util.AnalogItemUtil;
...
public class SubscriptionManager {
	...
	private static boolean subtypeOf(OpcUaServer server, NodeId dataTypeId, NodeId potentialSuperTypeId) {
		...
		if (dataTypeNode != null) {
			...
			if (superTypeId != null) {
				return superTypeId.equals(potentialSuperTypeId) || subtypeOf(server, superTypeId, potentialSuperTypeId);
			} else {
				return false;
			}
		} else {
	...
	public CompletableFuture<CreateSubscriptionResponse> createSubscription(CreateSubscriptionRequest request) {
		...
		var subscription = new Subscription(this, subscriptionId, request.getRequestedPublishingInterval(), request.getRequestedMaxKeepAliveCount().longValue(), request.getRequestedLifetimeCount().longValue(), request.getMaxNotificationsPerPublish().longValue(), request.getPublishingEnabled(), request.getPriority().intValue());
		...
		subscription.setStateListener((s, ps, cs) -> {
			if (cs == State.Closing) {
				...
				byMonitoredItemType(monitoredItems.values(), dataItems -> server.getAddressSpaceManager().onDataItemsDeleted(dataItems), eventItems -> server.getAddressSpaceManager().onEventItemsDeleted(eventItems));
		...
		var response = new CreateSubscriptionResponse(header, subscriptionId, subscription.getPublishingInterval(), uint(subscription.getLifetimeCount()), uint(subscription.getMaxKeepAliveCount()));
	...
	public CompletableFuture<ModifySubscriptionResponse> modifySubscription(ModifySubscriptionRequest request) {
		...
		var response = new ModifySubscriptionResponse(header, subscription.getPublishingInterval(), uint(subscription.getLifetimeCount()), uint(subscription.getMaxKeepAliveCount()));
	...
	public CreateMonitoredItemsResponse createMonitoredItems(ServiceRequestContext context, CreateMonitoredItemsRequest request) throws UaException {
		...
		Map<ReadValueId, AccessResult> accessResults = server.getAccessController().checkReadAccess(session, itemsToCreate.stream().map(MonitoredItemCreateRequest::getItemToMonitor).toList());

		List<MonitoredItemCreateResult> results = GroupMapCollate.groupMapCollate(itemsToCreate, createRequest -> accessResults.get(createRequest.getItemToMonitor()), accessResult -> group -> {
			if (accessResult instanceof AccessResult.Denied denied) {
				var result = new MonitoredItemCreateResult(denied.statusCode(), uint(0), 0.0, uint(0), null);
	...
	private List<MonitoredItemCreateResult> createMonitoredItems(Subscription subscription, TimestampsToReturn timestamps, List<MonitoredItemCreateRequest> requests) {
		...
		Map<NodeId, AttributesResponse> attributeResponses = readMonitoringAttributes(requests.stream().map(r -> r.getItemToMonitor().getNodeId()).toList());
		...
		for (MonitoredItemCreateRequest request : requests) {
			try {
				...
				if (globalCount <= globalMax && sessionCount <= sessionMax) {
					BaseMonitoredItem<?> monitoredItem = createMonitoredItem(request, subscription, timestamps, attributeResponses);

					monitoredItems.add(monitoredItem);

					results.add(new MonitoredItemCreateResult(StatusCode.GOOD, monitoredItem.getId(), monitoredItem.getSamplingInterval(), uint(monitoredItem.getQueueSize()), monitoredItem.getFilterResult()));
				} else {
				...
				results.add(new MonitoredItemCreateResult(e.getStatusCode(), UInteger.MIN, 0.0, UInteger.MIN, null));
			}
		}
	...
	private BaseMonitoredItem<?> createMonitoredItem(MonitoredItemCreateRequest request, Subscription subscription, TimestampsToReturn timestamps, Map<NodeId, AttributesResponse> attributeResponses) throws UaException {

		NodeId nodeId = request.getItemToMonitor().getNodeId();
		QualifiedName dataEncoding = request.getItemToMonitor().getDataEncoding();
		AttributeId attributeId = AttributeId.from(request.getItemToMonitor().getAttributeId()).orElseThrow(() -> new UaException(StatusCodes.Bad_AttributeIdInvalid));
		...
		if (attributeId == AttributeId.EventNotifier) {
			...
			Object filterObject = request.getRequestedParameters().getFilter().decode(server.getStaticEncodingContext());
			...
			try {
				revisedParameters = server.getAddressSpaceManager().onCreateEventItem(request.getItemToMonitor(), request.getRequestedParameters().getQueueSize());
			} catch (Throwable t) {
				throw new UaException(StatusCodes.Bad_InternalError, t);
			}

			MonitoredEventItem monitoredEventItem = new MonitoredEventItem(server, session, uint(subscription.nextItemId()), subscription.getId(), request.getItemToMonitor(), request.getMonitoringMode(), timestamps, request.getRequestedParameters().getClientHandle(), 0.0, revisedParameters.revisedQueueSize(), request.getRequestedParameters().getDiscardOldest());
			...
			try {
				revisedParameters = server.getAddressSpaceManager().onCreateDataItem(request.getItemToMonitor(), requestedSamplingInterval, request.getRequestedParameters().getQueueSize());
			} catch (Throwable t) {
				throw new UaException(StatusCodes.Bad_InternalError, t);
			}
			...
			if (isAnalogItem) {
				euRange = AnalogItemUtil.getEuRange(server, nodeIdMonitoredItem);
			}

			BaseMonitoredItem<?> monitoredItem;

			if (isAnalogItem) {
				monitoredItem = new AnalogItemMonitoredDataItem(server, session, uint(subscription.nextItemId()), subscription.getId(), request.getItemToMonitor(), request.getMonitoringMode(), timestamps, request.getRequestedParameters().getClientHandle(), revisedParameters.revisedSamplingInterval(), revisedParameters.revisedQueueSize(), request.getRequestedParameters().getDiscardOldest(), euRange);
			} else {
				monitoredItem = new MonitoredDataItem(server, session, uint(subscription.nextItemId()), subscription.getId(), request.getItemToMonitor(), request.getMonitoringMode(), timestamps, request.getRequestedParameters().getClientHandle(), revisedParameters.revisedSamplingInterval(), revisedParameters.revisedQueueSize(), request.getRequestedParameters().getDiscardOldest());
			}
	...
	private MonitoringFilter validateEventItemFilter(Object filterObject, MonitoringAttributes monitoringAttributes) throws UaException {
	...
	public ModifyMonitoredItemsResponse modifyMonitoredItems(ServiceRequestContext context, ModifyMonitoredItemsRequest request) throws UaException {
		...
		}).filter(NodeId::isNotNull).collect(toList());
		...
		for (int i = 0; i < itemsToModify.length; i++) {
			...
			try {
				BaseMonitoredItem<?> monitoredItem = modifyMonitoredItem(modifyRequest, timestamps, subscription, attributesMap);
	...
	private BaseMonitoredItem<?> modifyMonitoredItem(MonitoredItemModifyRequest request, TimestampsToReturn timestamps, Subscription subscription, Map<NodeId, AttributesResponse> attributeResponses) throws UaException {
		...
		AttributeId attributeId = AttributeId.from(monitoredItem.getReadValueId().getAttributeId()).orElseThrow(() -> new UaException(StatusCodes.Bad_AttributeIdInvalid));
		...
		if (attributeId == AttributeId.EventNotifier) {
			...
			try {
				revisedParameters = server.getAddressSpaceManager().onModifyEventItem(monitoredItem.getReadValueId(), parameters.getQueueSize());
			} catch (Throwable t) {
				throw new UaException(StatusCodes.Bad_InternalError, t);
			}
			...
			try {
				revisedParameters = server.getAddressSpaceManager().onModifyDataItem(monitoredItem.getReadValueId(), requestedSamplingInterval, requestedQueueSize);
			} catch (Throwable t) {
				throw new UaException(StatusCodes.Bad_InternalError, t);
			}

			monitoredItem.modify(timestamps, parameters.getClientHandle(), revisedParameters.revisedSamplingInterval(), filter, revisedParameters.revisedQueueSize(), parameters.getDiscardOldest());
		}
	...
	private Map<NodeId, AttributesResponse> readMonitoringAttributes(List<NodeId> nodeIds) {
		...
		List<DataValue> values = server.getAddressSpaceManager().read(context, 0.0, TimestampsToReturn.Neither, readValueIds);
		...
		for (int i = 0; i < values.size(); i += 4) {
			...
			if (dv0.statusCode().isBad()) {
				...
				var attributes = new MonitoringAttributes(nodeClass, eventNotifier, dataType, minimumSamplingInterval);
	...
	public CompletableFuture<PublishResponse> publish(ServiceRequestContext context, PublishRequest request) {
		...
		if (!transferred.isEmpty()) {
			...
			subscription.publishStatusChangeNotification(pending, new StatusCode(StatusCodes.Good_SubscriptionTransferred));
	...
	public CompletableFuture<SetTriggeringResponse> setTriggering(ServiceRequestContext context, SetTriggeringRequest request) {
		...
		synchronized (subscription) {
			...
			removeResults = Stream.of(linksToRemove).map(linkedItemId -> {
			...
			addResults = Stream.of(linksToAdd).map(linkedItemId -> {
		...
		var response = new SetTriggeringResponse(createResponseHeader(request), addResults, new DiagnosticInfo[0], removeResults, new DiagnosticInfo[0]);
	...
	private record MonitoringAttributes(NodeClass nodeClass, @Nullable UByte eventNotifier, @Nullable NodeId dataType, @Nullable Double minimumSamplingInterval) implements AttributesResponse {}
