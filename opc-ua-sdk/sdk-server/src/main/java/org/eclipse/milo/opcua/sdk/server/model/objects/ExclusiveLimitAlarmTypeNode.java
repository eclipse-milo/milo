...
public class ExclusiveLimitAlarmTypeNode extends LimitAlarmTypeNode
  ...
  public ExclusiveLimitStateMachineTypeNode getLimitStateNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "LimitState");
      var matches = new LinkedHashMap<NodeId, UaNode>();
      for (var reference : parent.getReferences()) {
        if (!reference.isForward()) {
          continue;
        ...
        if (!reference.getReferenceTypeId().equals(referenceId.orElseThrow())) {
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    + reference.getReferenceTypeId()
                    + " while resolving http://opcfoundation.org/UA/:LimitState (declaration"
    ...
    if (!(parent instanceof ExclusiveLimitStateMachineTypeNode)) {
      throw new UaRuntimeException(
          StatusCodes.Bad_TypeMismatch,
          "http://opcfoundation.org/UA/:LimitState (declaration i=9455, owner i=9341)"
              + " on "
    ...
    return (ExclusiveLimitStateMachineTypeNode) parent;
  }
}
