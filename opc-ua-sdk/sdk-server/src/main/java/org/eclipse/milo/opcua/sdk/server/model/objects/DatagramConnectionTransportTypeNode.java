...
public class DatagramConnectionTransportTypeNode extends ConnectionTransportTypeNode
  ...
  public @Nullable PropertyTypeNode getDiscoveryMaxMessageSizeNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "DiscoveryMaxMessageSize");
      var matches = new LinkedHashMap<NodeId, UaNode>();
      for (var reference : parent.getReferences()) {
        if (!reference.isForward()) {
          continue;
        ...
        if (!reference.getReferenceTypeId().equals(referenceId.orElseThrow())) {
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    + reference.getReferenceTypeId()
                    + " while resolving http://opcfoundation.org/UA/:DiscoveryMaxMessageSize"
  ...
  public @Nullable QosDataType @Nullable [] getDatagramQos() {
    ...
    QosDataType[] typed = new QosDataType[elements.length];
    for (int i = 0; i < elements.length; i++) {
      if (elements[i] != null && !(elements[i] instanceof QosDataType)) {
        throw new UaRuntimeException(
            StatusCodes.Bad_TypeMismatch,
            "http://opcfoundation.org/UA/:DatagramQos (declaration i=25526, owner i=15064)");
      }
      typed[i] = (QosDataType) elements[i];
    }
    return typed;
  }
  ...
  public NetworkAddressTypeNode getDiscoveryAddressNode() {
    UaNode parent = this;
    {
      ...
      var browseName = new QualifiedName(namespaceIndex, "DiscoveryAddress");
      var matches = new LinkedHashMap<NodeId, UaNode>();
      for (var reference : parent.getReferences()) {
        if (!reference.isForward()) {
          continue;
        ...
        if (!reference.getReferenceTypeId().equals(referenceId.orElseThrow())) {
          ...
          if (!referenceTypeTree.containsType(reference.getReferenceTypeId())) {
            throw new UaRuntimeException(
                ...
                "Unavailable ReferenceType "
                    + reference.getReferenceTypeId()
                    + " while resolving http://opcfoundation.org/UA/:DiscoveryAddress (declaration"
