 ...
 *
 * <p>Selected <code>BaseDataVariableType</code> contract over one retained backing node. Obtain
 * this view from a ServerViews context using {@link #TYPE}; the token itself does not prove
 * membership. Access after context close fails with IllegalStateException.
 */
@NullMarked
public final class BaseDataVariableTypeView extends ServerVariableView
    implements BaseDataVariableType {
  /** Selected UA/Java contract and context-owned factory. */
  public static final ServerVariableViewType<BaseDataVariableType> TYPE =
      ServerVariableViewType.of(
          ExpandedNodeId.parse("i=63"), BaseDataVariableType.class, BaseDataVariableTypeView::new);
  ...
  private BaseDataVariableTypeView(ServerViews views, UaVariableNode node) {
    super(views, node);
  }
}
