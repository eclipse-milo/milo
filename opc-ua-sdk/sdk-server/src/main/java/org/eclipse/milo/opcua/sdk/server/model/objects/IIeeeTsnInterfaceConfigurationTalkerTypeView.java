 ...
 */
@NullMarked
public final class IIeeeTsnInterfaceConfigurationTalkerTypeView extends ServerObjectView
    implements IIeeeTsnInterfaceConfigurationTalkerType {
  /** Selected UA/Java contract and context-owned factory. */
  public static final ServerViewType<IIeeeTsnInterfaceConfigurationTalkerType> TYPE =
      ServerViewType.of(
  ...
  private IIeeeTsnInterfaceConfigurationTalkerTypeView(ServerViews views, UaObjectNode node) {
    super(views, node);
  }
        ...
        ((Supplier<@Nullable UaVariableNode>)
                () -> {
                  UaNode parent = this.node;
                  {
                    ...
                    var browseName = new QualifiedName(namespaceIndex, "TimeAwareOffset");
                    var matches = new LinkedHashMap<NodeId, UaNode>();
                    for (var reference : parent.getReferences()) {
                      if (!reference.isForward()) {
                        continue;
                      ...
                      if (!reference.getTargetNodeId().isLocal()) {
                        throw new UaRuntimeException(
                            StatusCodes.Bad_NotSupported,
                            "http://opcfoundation.org/UA/:TimeAwareOffset (declaration i=24194,"
                                + " owner i=24191) on "
                                + getNodeId());
                      }
                      var targetId = reference.getTargetNodeId().toNodeId(namespaceTable);
   ...
   */
  @Override
  public @Nullable BaseDataVariableType getTimeAwareOffsetNode() {
    this.views.checkOpen();
    views.checkOpen();
   ...
   */
  @Override
  public @Nullable UInteger getTimeAwareOffset() {
    this.views.checkOpen();
    views.checkOpen();
    var child = viewMember2();
    if (child == null) {
   ...
   */
  @Override
  public void setTimeAwareOffset(@Nullable UInteger value) {
    this.views.checkOpen();
    views.checkOpen();
    var child = viewMember2();
    if (child == null) {
