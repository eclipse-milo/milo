...
class MethodBindingsTest {
  ...
  void sharedMethodDispatchesByObjectAndStaleTokensCannotRemoveReplacement() throws Exception {
    try (Fixture f = new Fixture();
      ...
      MethodInvocationHandler dispatcher = f.method.getInvocationHandler();
      MethodBinding replacement = bindings.bind(f.first, f.method, reply("new"));
      assertSame(dispatcher, f.method.getInvocationHandler());
      old.close();
      old.close();
  ...
  @Test
  void rawReplacementWinsAndDisplacedLifetimeCannotReattach() throws Exception {
    try (Fixture f = new Fixture();
        MethodBindings bindings = new MethodBindings()) {
      ...
      assertThrows(
          IllegalStateException.class, () -> bindings.bind(f.second, f.method, reply("late")));
      token.close();
      bindings.close();
      assertSame(raw, f.method.getInvocationHandler());
      try (MethodBindings fresh = new MethodBindings()) {
        fresh.bind(f.first, f.method, reply("fresh"));
        assertEquals("fresh", value(f.call(f.first)));
      }
      assertSame(raw, f.method.getInvocationHandler());
    }
  }
  ...
  @Test
  void competingRegistriesAreRejectedWithoutDispatcherChaining() throws Exception {
    try (Fixture f = new Fixture();
        MethodBindings first = new MethodBindings();
        MethodBindings second = new MethodBindings()) {
      first.bind(f.first, f.method, reply("first"));
      MethodInvocationHandler owner = f.method.getInvocationHandler();
      assertThrows(
          IllegalStateException.class, () -> second.bind(f.second, f.method, reply("second")));
      assertSame(owner, f.method.getInvocationHandler());
      second.close();
      assertEquals("first", value(f.call(f.first)));
    }
  }

  // A CAS loser must inspect the replacement again, never capture a competing dispatcher as
  // fallback.
  @Test
  void competingRegistryInstalledDuringCasRetryIsRejected() throws Exception {
    try (Fixture f = new Fixture();
        MethodBindings first = new MethodBindings();
        MethodBindings second = new MethodBindings()) {
      UaMethodNode method = spy(f.method);
      f.space.getNodeManager().addNode(method);
      AtomicBoolean inject = new AtomicBoolean(true);
      doAnswer(
              invocation -> {
                if (inject.getAndSet(false)) {
                  second.bind(f.second, method, reply("second"));
                  return false;
                }
                return invocation.callRealMethod();
              })
          .when(method)
          .compareAndSetInvocationHandler(any(), any());
      assertThrows(IllegalStateException.class, () -> first.bind(f.first, method, reply("first")));
  ...
  @Test
  void crossServerNodesAndCrossServerRegistryReuseAreRejected() throws Exception {
    try (Fixture f = new Fixture();
      ...
      bindings.bind(f.first, f.method, reply("first"));
      assertThrows(
          IllegalStateException.class,
          () -> bindings.bind(other.first, other.method, reply("wrong")));
    }
  }
    ...
    try (Fixture f = new Fixture();
      ...
      assertEquals(
          ...
          assertThrows(
              ...
              .getStatusCode());
      UaMethodNode other =
          UaMethodNode.builder(f.space.getNodeContext())
              .setNodeId(new NodeId(f.namespace, "other"))
              .setBrowseName(new QualifiedName(f.namespace, "Other"))
              .setDisplayName(LocalizedText.english("Other"))
              .buildAndAdd();
      assertEquals(
          new StatusCode(StatusCodes.Bad_TypeMismatch),
          assertThrows(
  ...
  @Test
  void rawReplacementAfterLastTokenReleasePreventsReattachment() throws Exception {
    try (Fixture f = new Fixture();
        MethodBindings bindings = new MethodBindings()) {
      bindings.bind(f.first, f.method, reply("bound")).close();
      MethodInvocationHandler replacement = reply("external");
      f.method.setInvocationHandler(replacement);
      assertThrows(
          IllegalStateException.class, () -> bindings.bind(f.first, f.method, reply("late")));
      bindings.close();
      assertSame(replacement, f.method.getInvocationHandler());
    }
  }

  @Test
  void displacementRefusesFurtherBindsWithoutRevokingOtherExistingRegistrations() throws Exception {
    try (Fixture f = new Fixture();
        MethodBindings bindings = new MethodBindings()) {
      UaMethodNode other =
          UaMethodNode.builder(f.space.getNodeContext())
              ...
              .setDisplayName(LocalizedText.english("Other"))
              .buildAndAdd();
      other.setOutputArguments(f.method.getOutputArguments());
      f.first.addComponent(other);
      MethodInvocationHandler otherFallback = reply("other fallback");
      other.setInvocationHandler(otherFallback);
      bindings.bind(f.first, f.method, reply("first"));
      bindings.bind(f.first, other, reply("other"));
      MethodInvocationHandler raw = reply("external");
      f.method.setInvocationHandler(raw);
      assertThrows(
          IllegalStateException.class, () -> bindings.bind(f.first, other, reply("replacement")));
      assertEquals(
          "other",
          value(
              f.space
                  .call(
                      new AddressSpace.CallContext(f.server, null),
                      List.of(
                          new CallMethodRequest(
                              f.first.getNodeId(), other.getNodeId(), new Variant[0])))
                  .get(0)));
      bindings.close();
      assertSame(raw, f.method.getInvocationHandler());
      assertSame(otherFallback, other.getInvocationHandler());
    }
  }
    ...
    try (Fixture f = new Fixture();
      ...
      UaReferenceTypeNode referenceType =
          new UaReferenceTypeNode(
              ...
              org.eclipse.milo.opcua.stack.core.types.builtin.unsigned.UInteger.MIN,
              org.eclipse.milo.opcua.stack.core.types.builtin.unsigned.UInteger.MIN,
              false,
              false,
      ...
      referenceType.addReference(
          new org.eclipse.milo.opcua.sdk.core.Reference(
              ...
              org.eclipse.milo.opcua.sdk.core.Reference.Direction.INVERSE));
      ...
      owner.addReference(
          new org.eclipse.milo.opcua.sdk.core.Reference(
              ...
              org.eclipse.milo.opcua.sdk.core.Reference.Direction.FORWARD));
    ...
    Fixture() {
      ...
      method =
          UaMethodNode.builder(space.getNodeContext())
              .setNodeId(new NodeId(namespace, "shared"))
              .setBrowseName(new QualifiedName(namespace, "Shared"))
              .setDisplayName(LocalizedText.english("Shared"))
              .buildAndAdd();
