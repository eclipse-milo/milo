...
On the consuming side the symmetry is deliberately looser. A reader group is organizational: it owns no socket and no thread — activating one validates its message security and ensures the connection's subscriber channel is open (a [secured group](message-security-and-sks.md) additionally stays PreOperational until its first key fetch completes), nothing more — and exists for lifecycle scoping, reconfigure restart scope, a diagnostics rollup level, and the message-security scope: the runtime resolves security per reader *group*, so readers with different security needs belong in different groups. The unit that matters is the DataSetReader: each reader filters on `publisherId` and `dataSetWriterId` (unset filters are wildcards) and yields one decoded stream. Readers do not need to mirror the publisher's group structure — and under the default UADP masks they cannot even see it, as the next section explains. One reader per writer, as in the example below, keeps the streams separated in application code; a single wildcard reader that receives every matching DataSetMessage can be the right shape for a generic monitor.
...
- Above the sending OS's limit — whether that is 65,507 or macOS's 9,216 — the failure is the same: the OS rejects the send with `java.net.SocketException: Message too long`. The trace is on the publisher's diagnostics: a `SEND_FAILURE` `PubSubDiagnosticsEvent` at the writer group's path with the message `failed to send NetworkMessage: Message too long`, the group's `lastError`, one `failedTransmissions` tick at the group, and one `failedDataSetMessages` tick per contributing writer. The event's status code is `Bad_InternalError` — a raw socket exception carries no OPC UA status code to extract, so the fallback code applies (transport-level failures that do carry one, like an MQTT `Bad_ServerNotConnected`, surface verbatim). No log line is emitted — and because `networkMessagesSent` / `dataSetMessagesSent` are incremented when the message is handed to the channel, before the asynchronous send resolves, the failed sends still count as sent; the `failedTransmissions` counter is what tells you they never left the machine. If you have no diagnostics listener registered, the counters and `lastError` in the diagnostics snapshot are the persistent evidence.
- With a writer-group `maxNetworkMessageSize` configured, the failure moves earlier and gets a better name: the oversize message is skipped at publish, before it ever reaches the socket, with `Bad_EncodingLimitsExceeded` at the group's path naming the actual and maximum sizes — and unlike the socket case, the skipped DataSetMessages are *not* counted sent. The skip ticks the same failure pair: `failedTransmissions` at the group and `failedDataSetMessages` at each contributing writer.
...
- The MQTT transport configures no client-side packet-size limit, so absent a writer-group `maxNetworkMessageSize` — which is enforced on MQTT exactly as on UDP, skipping the oversize message before it reaches the broker — the effective limit is whatever your broker enforces. A publish the broker or client rejects fails through the send future: `MqttPublisherChannel` logs it at DEBUG, and the engine records the same send-failure diagnostics as the UDP case, with the failure's own status code (`Bad_ServerNotConnected` while disconnected, `Bad_InternalError` for causes carrying no OPC UA code) — again, diagnostics, not logs, are where to look.
...
- Keep a dataset's encoded key frame within one MTU-sized datagram (~1400 bytes) where you can; the hard Milo-to-Milo UDP ceiling is the sending OS's datagram limit (~65,507 bytes on Linux, 9,216 on macOS/BSD at defaults), and everything between the two travels only by IP fragmentation. Nothing chunks on send, and nothing warns at config time that a dataset will be too big — set the group's `maxNetworkMessageSize` if you want oversize cycles skipped with a `Bad_EncodingLimitsExceeded` diagnostics error instead of discovered at the socket; on secured groups leave 46–47 bytes of headroom for the security overhead.
...
- Monitor diagnostics, not logs. Send failures of every kind surface as `SEND_FAILURE` `PubSubDiagnosticsEvent`s carrying their real status codes, tick `failedTransmissions` (group) and `failedDataSetMessages` (writers), and set `lastError` — the sent counters keep ticking through socket-level failures, though oversize-skipped messages are never counted sent — and receive-side failures tick `decodeErrors`: at the connection path for decode failures, at the reader-group path for size refusals.
