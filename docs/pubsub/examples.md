...
| MQTT, JSON + retained metadata/status | `mqtt.MqttJsonPublisherExample`, `mqtt.MqttJsonSubscriberExample` | JSON mapping; retained `ua-metadata` supplies field definitions and retained `ua-status`/MQTT Will reports publisher health | [MQTT transport](mqtt.md) |
