...
- Discovery is never secured. Probes and metadata announcements are always sent with security mode None, even when the groups they describe use [message security](message-security-and-sks.md) — spec-legal (no Part 14 rule assigns discovery messages a security group), but it means dataset metadata is not confidential. The discovery plane also resolves no keys, so a secured discovery message from a peer is skipped.
