...
| [Event](limitations-and-interop.md#events) publishing | Works — a `PublishedEventsConfig` source, the `publishEvent(...)` push API, and interval-0 event-triggered groups; UADP (`0x02`) and JSON (`ua-event`) encode; `ServerPubSub` bridges the server's event bus for you (one NetworkMessage per event, no batching yet) |
