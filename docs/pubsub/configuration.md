...
- an interval-0 (event-triggered) writer group that contains a data-items writer; positive-interval
  groups may contain event-source writers and drain their queued events on the scheduled publish
  cycle (see [publishing events](#publishing-events-the-push-api-and-event-triggered-groups))
- an enabled event-source writer whose group sets a non-zero UADP `ConfiguredSize` (fixed-size
  layouts are incompatible with event Variant encoding), or a JSON event writer whose masks omit
  the `DataSetMessageHeader`/`MessageType` members event messages need
...
## The published dataset source: data items or events

A `PublishedDataSetConfig` no longer holds only a field list. Its *source* is polymorphic —
`PublishedDataSetSourceConfig` is a sealed interface with two implementations:

- **`PublishedDataItemsConfig`** — the ordered `FieldDefinition` list every earlier example uses:
  a cyclic dataset whose fields are sampled once per publishing interval. This is the default. The
  `PublishedDataSetConfig.Builder.field(FieldDefinition)` sugar builds one implicitly, so
  `PublishedDataSetConfig.builder("telemetry").field(...).field(...).build()` is unchanged; it is
  equivalent to setting `source(PublishedDataItemsConfig.builder()...build())`.
- **`PublishedEventsConfig`** — an *event* dataset, populated by events from an event notifier Node
  rather than sampled on a cycle.

The builder enforces one rule: `field(...)` and `source(...)` are mutually exclusive — passing both
fails `build()` — because `field(...)` is itself shorthand for a data-items source. Set exactly one.
`PublishedDataSetConfig.getSource()` returns the source; `getFields()` still works, returning the
data-items field list or an empty list for an event source.

An event source is built with `PublishedEventsConfig.builder(eventNotifier)`:

```java
PublishedDataSetConfig alarms =
    PublishedDataSetConfig.builder("alarms")
        .source(
            PublishedEventsConfig.builder(eventNotifier)
                .field(
                ...
                .field(
                    EventFieldDefinition.builder("Severity")
                        ...
                        .build())
                // .filter(...) defaults to new ContentFilter(null), which matches every event
                .build())
        .build();
```
...
- **`eventNotifier`** is an `ExpandedNodeId`, the Node whose events feed the dataset (the server's
  `Server` object, `i=2253`, is the common choice — it sees every event). Author it in the canonical
  namespace-URI form `NodeId.expanded(namespaceTable)` produces, because that is the form the Part 14
  import mapping yields, so an imported config compares equal to its authored counterpart.
- **`EventFieldDefinition`s** are the dataset's fields, in wire order. Each names a field, carries a
  `SimpleAttributeOperand` (`selectedField`) selecting the event field to publish, and metadata used
  to derive the dataset's `DataSetMetaData`: `dataType` (default `BaseDataType`), `valueRank`
  (default `-1`, scalar), `arrayDimensions`, a `promoted` flag, a stable `dataSetFieldId` (random per
  build if unset), and extra `properties`. `build()` rejects duplicate field names and duplicate
  `dataSetFieldId`s. A zero-field event dataset is legal but publishes nothing.
- **`filter`** is a where-clause `ContentFilter`; the default, `new ContentFilter(null)`, matches
  every event.

One portability caveat: NodeIds *inside* the `selectedField` operands and the `filter` are carried
verbatim and are **not** remapped across namespace tables, so event configs whose selectors or
filters reference non-zero namespace indices are not portable across processes with differing
namespace tables (the `eventNotifier` itself *is* remapped). See
[limitations](limitations-and-interop.md#events).

### Publishing events: the push API and event-triggered groups

Events are pushed, not sampled. `PubSubService.publishEvent(PublishedDataSetRef dataSet,
List<Variant> fields)` offers one event — its field values in the dataset's configured field order —
to every enabled event-source writer referencing that dataset (Part 14 §6.2.6.2). It throws
`IllegalArgumentException` if the ref is unknown, is not an event source, or the field count differs
from the dataset's event-field count; an event whose referencing writers are all `Disabled`, paused
outside an active parent writer group, or not yet constructed is dropped. Enabled writers paused
under a `PreOperational` or recoverable `Error` writer group still buffer events, which is what
preserves events pushed while a secured group waits for its first key fetch. When PubSub is attached
to an `OpcUaServer`, the
[event-notifier binding](server-integration.md#publishing-events-the-event-notifier-binding) calls
`publishEvent` for you from the server's event bus.

Each writer buffers events in a bounded per-writer queue sized by
`DataSetWriterConfig.eventQueueCapacity` (default 100, `build()` rejects values below 1; ignored for
data-items writers). On overflow the writer synthesizes an `EventQueueOverflowEventType` event as the
queue's last entry and discards the events that do not fit, per §6.2.6.2.

Because an event dataset is non-cyclic, its writers can run in either of two timing modes:

- In a positive-interval group, `publishEvent` queues events and the regular publish cycle drains
  them on the next scheduled tick. Positive-interval groups may mix data-items and event-source
  writers.
- In an interval-0 group, the group is *event-triggered*: it runs no fixed-rate publish task, every
  writer must reference an event-source dataset, and each `publishEvent` schedules an immediate
  drain. `WriterGroupConfig.Builder.build()` rejects only *negative* intervals now; zero is legal.
  The typed config builders reject a data-items writer in an interval-0 group with
  `PubSubConfigValidationException`; startup and reconfigure keep the same invariant as a
  `Bad_ConfigurationError` backstop for any config that reaches the runtime.

Part 14 requires `keyFrameCount` 0 for event writers. The config tolerates any value on import, but
export normalizes an event writer's `keyFrameCount` to 0. In both timing modes, every buffered event
becomes its own NetworkMessage (see
[limitations](limitations-and-interop.md#events) for the one-NetworkMessage-per-event trade-off).

If the group is [secured](message-security-and-sks.md), configure a `keepAliveTime` on it. A secured
interval-0 group whose security key fetch is skipped for a cycle leaves its events queued until the
next event, keep-alive tick, or return to `Operational`; a positive `keepAliveTime` bounds that
recovery delay (see [operations](operations.md#timing-behavior-worth-knowing)). Without a fixed-rate
task, a keep-alive is the only periodic signal an interval-0 group has.
...
carrying only the fields that changed. The meaning of 0 depends on the writer's dataset. For a
data-items writer, 0 is the spec's non-cyclic value: Milo clamps it to 1 at runtime, so 0 and 1 both
mean every-message-key-frame and imported configs round-trip unmodified. For an
[event-source writer](#the-published-dataset-source-data-items-or-events) the dataset is genuinely
non-cyclic — `keyFrameCount` is ignored, Part 14 requires it to be 0, and export normalizes any
value to 0.
