...
- an interval-0 (event-triggered) writer group that contains a data-items writer, or an
  event-source writer in a group with a positive `publishingInterval` — the two must match (see
  [publishing events](#publishing-events-the-push-api-and-event-triggered-groups))
...
(or not yet constructed) is dropped. When PubSub is attached to an `OpcUaServer`, the
...
Because an event dataset is non-cyclic, its writers live in a writer group with a **zero**
`publishingInterval` — an *event-triggered* group. `WriterGroupConfig.Builder.build()` rejects only
*negative* intervals now; zero is legal. The pairing is validated as a cross-rule:

- Every writer in an interval-0 group must reference an event-source dataset (and Part 14 requires
  `keyFrameCount` 0 on each — the config tolerates any value on import, but export normalizes an
  event writer's `keyFrameCount` to 0). A data-items writer in an interval-0 group, or an
  event-source writer in a group with a positive interval, is rejected with `Bad_ConfigurationError`
  at build, startup, and reconfigure — and a data-items writer enabled *into* an interval-0 group
  after startup fails into PubSubState `Error` at activation.
- An event-triggered group runs no fixed-rate publish task. Each `publishEvent` schedules an
  immediate drain, and every buffered event becomes its own NetworkMessage (see
...
value to 0 (an interval-0 event-triggered group requires 0 on all its writers).
