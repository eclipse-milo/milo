 ...
 *
 * <p>What it demonstrates — the broker metadata path:
 *
 * <ul>
 ...
 *   <li>logging metadata arrival via {@code addMetaDataListener} and received DataSets via a
 *       pre-wired per-reader listener.
 * </ul>
 *
 * <p>Terminal order:
 *
 ...
 * ua-metadata} document is replayed, then one "[received] ..." line per second naming the
...
public class MqttJsonSubscriberExample {
  ...
  // The publisher's derived Part 14 topics: <prefix>/<mapping>/<data|metadata>/<publisherId>/
  // <writerGroupName>[/<dataSetWriterName>]. These must match MqttJsonPublisherExample
  ...
  private void run(URI brokerUri) throws Exception {
    ...
    logger.info(
        "subscriber started: broker={}, data topic={}, metadata topic={}",
        brokerUri,
        DATA_TOPIC,
        META_TOPIC);
